...
import com.bootfactory.microinformes.application.modelDTO.ReporteVMDDTO;
...
@RestController
@RequestMapping("/reporteEstadoVMD")
...
public class VisEstadoVMDController {

	
	private final IVisEstadoVMDService service;

	public VisEstadoVMDController(IVisEstadoVMDService service) {
		this.service = service;
	}

	
	@Operation(description = "Obtiene la lista  de todos los datos VMD para los deudorres filtrado por  id de la entidad financiera")
	@GetMapping("/{idEntidadFinanciera}/{tipoIdentificacion}/{identificacion}/{registroInicial}/{numeroRegistros}")
	public ResponseEntity<ReporteVMDDTO> generarReporte(
			...
			@Parameter(name = "tipoIdentificacion", description = "Código de tipo de identificación del deudor.  Si la consulta no se realizapor cliente debe tener valor 0. Ejemplos: 1 NIT , 2 Cédula de Ciudadanía, 3	Cédula Extranjería Residente, 4	Cédula Extranjería No Residente, 5	Tarjeta de Identidad, 6	Cédula de Extranjería, 7	Pasaporte, 8	Registro Civil", required = true) @PathVariable("tipoIdentificacion") Long tipoIdentificacion,
			@Parameter(name = "identificacion", description = "Número de identificación del deudor. Si la consulta no se realizapor cliente debe tener valor 0", required = true) @PathVariable("identificacion") String identificacion,
	...
	private ResponseEntity<ReporteVMDDTO> listarPaginado(Long idEntidadFinanciera, Long tipoIdentificacion,
			String identificacion, int registroInicial, int numeroRegistros) {
		Long nroTotalRegistros;
		List<VisEstadoVMDDTO> datos;
		...
		ReporteVMDDTO reporte= new ReporteVMDDTO(datos,nroTotalRegistros);
		return ResponseEntity.ok(reporte);
	}

	private ResponseEntity<ReporteVMDDTO> listarNoPaginado(Long idEntidadFinanciera, Long tipoIdentificacion,
